...
- [-] 1. Scaffold the `track_aiops` package
...
- [ ] 2. Implement configuration loading with fail-fast validation
  - [~] 2.1 Create `track_aiops/config.py` with the `Settings` dataclass and `load()` function
  ...
  - [~] 2.2 Write property-based test for config fail-fast behavior
...
- [ ] 3. Implement the Bedrock client transport layer
  - [~] 3.1 Create `track_aiops/bedrock_client.py` with `BedrockResponse`, `BedrockClient`, and error classes
  ...
  - [~] 3.2 Write property-based test for request body construction
  ...
  - [~] 3.3 Write property-based test for response parsing
  ...
  - [~] 3.4 Write test for malformed response and auth error rejection
...
- [ ] 4. Implement Datadog LLM observability integration
  - [~] 4.1 Create `track_aiops/observability.py` with `enable_llmobs`, `traced_llm`, and `flush`
  ...
  - [~] 4.2 Write property-based test for span annotation completeness
  ...
  - [~] 4.3 Write test for enable and flush behavior
...
- [ ] 5. Implement the CLI entry point
  - [~] 5.1 Create `track_aiops/cli.py` with `main()` orchestration
  ...
  - [~] 5.2 Write integration test for CLI flow with mocks
